...
struct UserDeletionTests: AppQueueTests {

	@Test("Test User Deletion cleans up awards on Disk")
	func awardFileDeletedOnUserDelete() async throws {
		try await withApp { app in
			var cookies: HTTPCookies?
			try await app.test(.POST, "credentials/register") {
				req async throws in
				try req.content.encode(
					CredentialsAuthentificationController
						.CredentialsRegisterContent(
							accountType: .licensed,
							callsign: "DL2TEST",
							acceptTerms: "on",
							password: "VeryS3cre7",
							password_repeat: "VeryS3cre7"
						)
				)
			...
			let user = try #require(
				await UserModel.userFor(callsign: "DL2TEST", on: app.db).get()
			)
			...
			try fileManager.createDirectory(
				atPath: fullAwardContainerPath,
				withIntermediateDirectories: true
			)
			#expect(
				fileManager.createFile(atPath: fullAwardPath, contents: Data())
			)
			...
			#expect(
				try await UserModel.userFor(callsign: "DL2TEST", on: app.db)
					.get() == nil
			)
		}
	}
