...
public class DARCSSORouter: FederatedServiceRouter {
	...
	public var requiredScopes: [String] = [ "openid" ]
	public let callbackURL: String
	public let accessTokenURL: String
	public var service: OAuthService = .darcSSO
	...
	private static let verifierKey = "darc-sso-verifier"
	private static let stateKey = "darc-sso-state"
	...
	public required init(callback: String, completion: @escaping (Request, String) throws -> (EventLoopFuture<ResponseEncodable>)) throws {
		let auth = try DARCSSOAuth()
		self.tokens = auth
		self.baseURL = "https://sso.darc.de/module.php/oidc"
		self.accessTokenURL = baseURL.finished(with: "/") + "token.php"
		self.callbackURL = callback
		self.callbackCompletion = completion
	}

	public func authURL(_ request: Request) throws -> String {
		let path="authorize.php"

		let state = UUID().uuidString
		request.session.data[Self.stateKey] = state
...
//				request.session.setRefreshToken(refreshToken)
	...
	public func callbackBody(with code: String, for request: Request) throws -> ResponseEncodable {
		// TODO: verify state key was the same as the one where the user left to DARC SSO
		guard let state = request.session.data[Self.stateKey] else {
			throw Abort(.badRequest, reason: "Missing state.")
		}
		request.session.data[Self.stateKey] = nil
		guard try state == request.query.get(String.self, at: "state") else {
			throw Abort(.badRequest, reason: "State query value does not match known value.")
		}
		...
		return DARCSSOCallbackBody(clientId: self.tokens.clientID,
